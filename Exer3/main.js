...
function updateScore() {
    const scoreElement = document.getElementById("placar");
    scoreElement.textContent = `${pointsE} x ${pointsD}`;
}

function atualizaAnimacao(){
    
    let raio = 0.05;
    let larguraBarra = 0.05;
    let alturaBarra = 0.2;

    
    if(txBola + raio >= 0.9 - larguraBarra && txBola - raio <= 0.9 + larguraBarra){         // Colisão com a barra direita
        if(tyBola - raio <= tyBD + alturaBarra && tyBola + raio >= tyBD - alturaBarra){
            if(txBola_offset > 0){
    ...
    if(txBola + raio >= -0.9 - larguraBarra && txBola - raio <= -0.9 + larguraBarra){       // colisão com barra esquerda
        if(tyBola - raio <= tyBE + alturaBarra && tyBola + raio >= tyBE - alturaBarra){
            if(txBola_offset < 0){
                txBola_offset = -txBola_offset;
            }
        }
